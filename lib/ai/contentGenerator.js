...
export class ContentGenerator {
  constructor(apiKey) {
    // Let the library use its default endpoint configuration
    // The latest version (4.13.11+) handles endpoint routing automatically
    this.hf = new HfInference(apiKey);
    this.model = 'mistralai/Mistral-7B-Instruct-v0.2'; // Free model, can be changed
  }

  async generateArticle(topic, facts, relatedData) {
    try {
      const prompt = this.buildArticlePrompt(topic, facts, relatedData);
