...
export async function getTopicsToProcess() {
  ...
  if (trendsWithoutArticles.length > 0) {
    ...
    const stats = { totalTrends, trendsWithArticles };
    if (validatedTrends.length > 0) {
      return { topics: validatedTrends, source: 'trends', stats };
    }
    // All trends failed validation - return empty with stats so cron response explains why
    return {
      topics: [],
      source: 'trends',
      stats: { ...stats, trendsRejectedByValidation: trendsWithoutArticles.length },
    };
  }
