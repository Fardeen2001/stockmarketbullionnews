...
export class ContentHumanizer {
   ...
   * Splits by paragraph boundaries (\\n\\n) to preserve ## h2 and ### h3 headings.
   * Chunks are built from whole paragraphs so heading structure is never broken.
   * @param {string} content - Full content
   * @param {number} chunkSize - Approximate chars per chunk (default: 2000)
  ...
  async humanizeInChunks(content, chunkSize = 2000) {
    ...
    const humanizedChunks = await Promise.all(chunks.map((chunk) => this.humanize(chunk)));
