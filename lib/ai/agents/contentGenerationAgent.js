...
import { HUGGINGFACE_CONFIG, getHfInferenceConfig } from '@/lib/ai/config';
...
export class ContentGenerationAgent extends BaseAgent {
  ...
  async retrieveContext(topic, relatedSymbols, relatedMetals) {
    ...
    // Search for similar scraped content (with error handling)
    let similarScraped = [];
    try {
      ...
      // Continue with empty results - article can still be generated
    }
  ...
  buildRAGPrompt(topic, contextData) {
    ...
    const sourcesList = contextData.facts
      .map((f, i) => `${i + 1}. ${f.title} - ${f.sourceUrl || f.source}`)
      .join('\n');

    return `You are a world-class financial content writer with 10+ years of experience in SEO-optimized content creation for stock markets and precious metals. You have deep expertise in:
...
Article:`;
  ...
  injectExternalLinksAutomatically(content, contextData, externalLinks, usedLinkPositions) {
    ...
    const availableSources = contextData.facts
