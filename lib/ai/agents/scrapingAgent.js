...
export class ScrapingAgent extends BaseAgent {
  ...
  async execute(task, context = {}) {
    ...
    try {
      const { sources, maxItems = 50 } = task;
      const scrapedItems = [];
      ...
      // Process and deduplicate using vector search
      const processed = await this.processAndDeduplicate(scrapedItems);
  ...
  async processAndDeduplicate(items) {
    const processed = [];
    const seen = new Set();
    ...
    for (const item of items) {
      ...
      const similar = await this.vectorDB.searchSimilar(
        ...
        embedding,
        5,
        0.85 // 85% similarity threshold for deduplication
      );

        // Check if this is a duplicate
        const isDuplicate = similar.some(s => {
          return s.similarity > 0.85;
        });

      if (!isDuplicate && !seen.has(item.sourceUrl)) {
