 ...
 * Sequence: Scrape → Detect trends → Generate articles → Index → Revalidate sitemap
 */
export async function runFullWorkflow() {
  ...
  try {
    ...
    if (articleResult.fatal) {
      logger.error('Full workflow: article generation aborted', {
        message: articleResult.message,
      });
    } else {
  ...
  // Step 4: Index new articles to search engines (GSC, Bing, Yandex)
  ...
  try {
    ...
    logger.info('Full workflow: indexing completed', results.indexing);
  } catch (err) {
    results.indexing = { success: false, error: err.message };
    logger.error('Full workflow: indexing failed', { error: err.message });
  } finally {
    await indexer.close();
  }

  // Step 5: Revalidate sitemap and news pages
  const revalidated = [];
  try {
