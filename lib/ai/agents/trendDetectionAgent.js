...
export class TrendDetectionAgent extends BaseAgent {
  ...
  async getRecentContent(hours, category = null) {
    ...
    // Mutually exclusive categories: metals first (precious metals content), then stocks (equities, no metals), then sharia (verified halal symbols)
    if (category === 'metals') {
      query.$or = [
      ...
      query.$and = [
        ...
        // Exclude metal-heavy content so stocks and metals are mutually exclusive
        {
          $or: [
