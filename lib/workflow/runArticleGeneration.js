 ...
 * @returns {Promise<{ success: boolean, fatal?: boolean, generated: number, skipped: number, errors: number, totalTopics: number, articles: Array, message: string, source?: string, stats?: Object }>}
 */
export async function runArticleGeneration({ hfApiKey, model } = {}) {
  try {
    return await runArticleGenerationCore({ hfApiKey, model });
  } catch (error) {
    const message = error?.message || String(error) || 'Article generation aborted';
    logger.error('runArticleGeneration failed', {
      error: message,
      stack: error?.stack,
    });
    return {
...
async function runArticleGenerationCore({ hfApiKey, model } = {}) {
  ...
  if (topicsToProcess.length === 0) {
    ...
    const rejected = stats?.trendsRejectedByValidation ?? 0;
    let message =
      totalTrends === 0
    ...
    if (rejected > 0) {
      message += ` ${rejected} trend(s) failed validation (e.g. unknown symbol or generic topic).`;
    }
    return {
      success: true,
      generated: 0,
