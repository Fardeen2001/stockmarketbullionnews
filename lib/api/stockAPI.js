...
export class YahooFinanceAPI {
  ...
  async getAllBSEStocks() {
    ...
    const bseEndpoints = [
      ...
      // Alternative: Try BSE equity list endpoint
      {
      ...
      try {
        ...
        if (response.status === 200 && response.data) {
          ...
          if (Array.isArray(data)) {
            data.forEach(stock => {
              const symbol = stock.SCRIP_CD || stock.SCRIP_ID || stock.symbol || stock.SYMBOL;
              if (symbol && typeof symbol === 'string' && symbol.length > 0 && !stocks.has(symbol)) {
                stocks.set(symbol, {
                  symbol: symbol,
                  exchange: 'BSE',
                  name: stock.SCRIP_NAME || stock.COMPANY_NAME || stock.companyName || stock.name || symbol,
                });
              }
            });
          ...
          } else if (typeof data === 'string' && data.includes('SCRIP_CD')) {
            // Try to parse HTML/CSV if returned as string
            // This is a fallback for HTML responses
            console.log('BSE API returned HTML/text format, skipping parsing (will use discovery method)');
          }
        }
        ...
        if (error.response) {
          // HTTP error (4xx, 5xx)
          console.debug(`BSE API endpoint ${endpoint.url} returned status ${error.response.status}`);
        } else if (error.request) {
          // Network error
          console.debug(`BSE API endpoint ${endpoint.url} network error: ${error.message}`);
        } else {
          // Other error
          console.debug(`BSE API endpoint ${endpoint.url} error: ${error.message}`);
        }
        continue;
      }
    }
    ...
    if (stocks.size > 0) {
      console.log(`Successfully fetched ${stocks.size} BSE stocks from official APIs`);
      return Array.from(stocks.values());
    }

    // This is expected - BSE APIs often require authentication or have CORS restrictions
    // The discovery method will be used as fallback
    console.log('Could not fetch BSE stocks from official APIs (this is normal). Will use discovery method.');
    return [];
  }
