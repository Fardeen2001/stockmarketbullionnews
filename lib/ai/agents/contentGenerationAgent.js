  ...
  async generateArticle(topic, contextData) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (wordCount < 400 || !hasHeadings) {
          throw new Error(`AI output too short (${wordCount} words) or missing headings`);
        }
...
2. CONTENT STRUCTURE - CRITICAL FOR SEO (600-800 words minimum):
   - MUST use ## for main section headings (h2) and ### for subsections (h3). This is essential for SEO.
   - Required heading structure: Start with intro paragraph(s), then:
     ## Market Overview (or similar)
     ## Key Drivers and Factors
     ## Impact on Investors (with ### subheadings for specific points if needed)
     ## Conclusion or Outlook
   - Use 4-6 ## headings minimum. Add ### subheadings where it improves readability.
   - Each ## heading must be on its own line, followed by 2-3 paragraphs of content.
   - Include semantic keyword variations in headings (e.g., ## Gold Price Trends in India)
   - Use natural, flowing paragraphs (3-5 sentences each) with smooth transitions
   - No other markdown (no bullet lists, no bold/italic); ONLY ## and ### for headings
...
- Output ONLY the article body with link markers and section headings
- Do NOT include title or summary
- MANDATORY: Use ## for every main section (h2) and ### for subsections (h3). Headings are critical for SEO.
- Example structure: [intro paragraphs] then ## First Section Title [content] then ## Second Section Title [content]
- Write in paragraphs with natural link markers embedded
- Ensure headings include target keywords where natural (e.g., ## ${topic} Market Analysis)
