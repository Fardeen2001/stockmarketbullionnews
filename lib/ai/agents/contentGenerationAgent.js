...
export class ContentGenerationAgent extends BaseAgent {
  ...
  async generateArticle(topic, contextData) {
    try {
      ...
      // Generate article using LLM (provider: hf-inference - Mistral needs text-generation, featherless-ai only supports conversational)
...
2. CONTENT STRUCTURE (600-800 words minimum):
   - Use SECTION HEADINGS for readability: ## for main sections (renders as h2), ### for subsections (renders as h3)
   - Example: ## Market Overview then ## Key Drivers then ### Impact on Investors
   - Use natural, flowing paragraphs (3-5 sentences each) with smooth transitions between sections
   - Structure: Introduction → ## Section One → ## Section Two → ### Subsections where needed → Conclusion
...
IMPORTANT: 
- Output ONLY the article text with link markers and section headings (## and ###)
...
- Ensure links and headings enhance readability, not distract from it
